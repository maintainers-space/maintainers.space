...
export default defineEventHandler((event) => {
  ...
  if (!clientId) {
    throw createError({ statusCode: 501, statusMessage: 'Codeberg OAuth is not configured on this server.' })
  }
  ...
  setCookie(event, 'cb_oauth', JSON.stringify({ state, returnTo, did }), {
    httpOnly: true,
    sameSite: 'lax',
