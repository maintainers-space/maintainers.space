...
export function useNotifications() {
  ...
  async function reply(item: ForgeInboxItem, body: string): Promise<boolean> {
    const forge = getForge(item.provider)
    // Repository-scoped requests (posting a comment) must use a repository
    // override token when one exists — e.g. one stored during 403 recovery —
    // and fall back to the provider-wide token otherwise.
    const token = getToken(
      item.provider,
      item.repo ? `${item.repo.owner}/${item.repo.name}` : undefined
    )
