...
const to = computed(() => `/${props.repo.provider}/${encodeURIComponent(props.repo.owner)}/${encodeURIComponent(props.repo.name)}`)
</script>
...
<template>
  <RepoContextMenu :repo="repo">
    ...
    >
      ...
      <p v-if="repo.description" class="mt-1 line-clamp-2 text-sm text-muted">{{ repo.description }}</p>
      <div class="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted">
        ...
        <span v-for="t in (repo.topics ?? []).slice(0, 3)" :key="t" class="rounded-full bg-elevated px-2 py-0.5">{{ t }}</span>
      </div>
    </NuxtLink>
  </RepoContextMenu>
</template>
