...
const props = withDefaults(defineProps<{ title: string, icon: string, count: number, total?: number }>(), {
  total: undefined
})
const open = ref(true)
</script>
  ...
  <section v-if="count > 0" class="space-y-3">
    ...
    >
      ...
      <UBadge color="neutral" variant="subtle" size="xs">
        {{ props.total != null && props.total > count ? `${count} of ${formatCompactNumber(props.total)}` : count }}
      </UBadge>
    </button>
