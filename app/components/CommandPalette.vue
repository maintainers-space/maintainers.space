    ...
    out.push({
      ...
      items: [
        ...
        ...(isAuthenticated.value
          ? [
              { label: 'Notifications', icon: 'i-lucide-bell', onSelect: () => go('/notifications') },
              { label: 'Linked accounts', icon: 'i-lucide-link', onSelect: () => go('/settings/accounts') },
      ...
      >
        <template #empty>
          <div class="py-8 text-center text-sm text-muted">
            ...
            <p v-if="term.trim()">
              Press <UKbd value="enter" /> to search everywhere.
            </p>
            <p v-else>
              Type to search across every forge.
            </p>
          </div>
        </template>
      </UCommandPalette>
