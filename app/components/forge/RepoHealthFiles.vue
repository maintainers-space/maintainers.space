...
const tabItems = computed(() => props.files.map(f => ({ label: f.label, icon: f.icon, value: f.path })))
...
interface DocState { loading: boolean, error: string | null, content: string }
...
<template>
  <div class="overflow-hidden rounded-lg border border-default">
    ...
    <div class="p-6">
      ...
      <MarkdownBody v-else-if="isMarkdown" :content="current?.content" empty="This file is empty." />

      <pre v-else class="overflow-x-auto whitespace-pre-wrap font-mono text-xs text-muted">{{ current?.content }}</pre>
    </div>
  </div>
</template>
