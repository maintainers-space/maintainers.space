...
<template>
  <div class="overflow-hidden rounded-lg border border-default">
    <div v-for="(c, i) in commits" :key="c.sha || i" :class="{ 'border-t border-default': i > 0 }">
      <CommonOfflineLink
        :to="commitPath(c.sha)"
        :cache-key="`commit:${provider}:${owner}:${repo}:${c.sha}`"
        class="flex items-center gap-3 px-4 py-3"
      >
        <template #default="{ offline }">
          <UIcon
            :name="offline ? 'i-lucide-wifi-off' : 'i-lucide-git-commit-horizontal'"
          ...
          <div class="min-w-0 flex-1">
            ...
            <div class="mt-0.5 flex items-center gap-2 text-xs text-muted">
              <span v-if="c.author">{{
                c.author.name || userLabel({ login: c.author.login })
              }}</span>
              <span v-if="c.author?.when">· {{ formatRelativeTime(c.author.when) }}</span>
            </div>
          </div>
          <span
            class="shrink-0 rounded border border-default px-2 py-0.5 font-mono text-xs text-muted"
          >
            {{ c.shortSha }}
          </span>
        </template>
