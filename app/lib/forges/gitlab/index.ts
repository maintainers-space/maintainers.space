  ...
  features: {
    ...
    pullRead: {
      ...
      async getMergeQueue(repo, branch, opts): Promise<ForgeMergeQueueStats | null> {
        ...
        try {
          cars = await glFetch<GlMergeTrainCarResponse[]>(
            path,
            { scope: 'active', per_page: 50 },
            opts
          )
        } catch {
          return null
    ...
    notificationRead: {
      ...
      async listInbox(opts): Promise<ForgeInboxItem[]> {
        ...
        const items = await glMapLimit(
          todos ?? [],
          6,
          async (t): Promise<ForgeInboxItem | null> => {
            const kind = todoKind(t)
            const { owner, name, fullName } = todoRepo(t)
    ...
    write: {
      async createComment(repo, id, body, opts): Promise<ForgeComment> {
        ...
        for (const kind of ['merge_requests', 'issues'] as const) {
          try {
            const note = await $fetch<GlNoteResponse>(
              `${API}/projects/${pid}/${kind}/${id}/notes`,
              {
                method: 'POST',
                headers: glHeaders(opts),
                body: { body },
      ...
      async setStar(repo, starred, opts): Promise<{ starred: boolean; stars?: number }> {
        try {
          ...
          return {
            starred,
            stars: typeof proj?.star_count === 'number' ? proj.star_count : undefined
          }
        } catch (e) {
