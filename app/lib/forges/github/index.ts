  ...
  features: {
    ...
    issueRead: {
      async listIssues(repo, opts) {
        const limit = opts?.limit ?? 30
        const page = opts?.cursor ? Number(opts.cursor) : 1
        const data = await $fetch<GhIssueResponse[]>(
          `${API}/repos/${repo.owner}/${repo.name}/issues`,
          {
            headers: ghHeaders(opts),
    ...
    pullRead: {
      async listPulls(repo, opts) {
        ...
        const data = await $fetch<GhPullResponse[]>(
      ...
      repository(owner:$owner,name:$name){
        ...
        if (!mq) return null
        const nodes = (mq.entries?.nodes ?? []).filter(
          (n): n is GhGraphqlMergeQueueEntryNode => !!n
        )
    ...
    actionRead: {
      ...
      async getActionRun(repo, id, opts) {
        const [run, jobs] = await Promise.all([
          $fetch<GhActionRunResponse>(
            `${API}/repos/${repo.owner}/${repo.name}/actions/runs/${id}`,
            {
              headers: ghHeaders(opts),
              signal: opts?.signal
            }
    ...
    activityRead: {
      ...
      async listMyWork(opts): Promise<ForgeMyWork> {
        ...
        const run = (q: string): Promise<ForgeIssue[]> =>
          githubProvider.features.search!.searchIssues!(q, {
            token,
            sort: 'updated',
            order: 'desc',
            limit: 8
