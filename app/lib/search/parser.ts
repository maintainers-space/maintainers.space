...
function tokenize(input: string): RawToken[] {
  ...
  while (i < n) {
    ...
    if (c === '(') { tokens.push({ t: 'lparen', v: '(' }); i++; continue }
    if (c === ')') { tokens.push({ t: 'rparen', v: ')' }); i++; continue }
    if (c === '|') { tokens.push({ t: 'or', v: 'OR' }); i++; continue }

    let negated = false
    if (c === '-') {
      const nx = input[i + 1]
      if (nx && !isSpace(nx) && nx !== '-' && nx !== '(') { negated = true; i++ } else { i++; continue }
    }
    ...
    if (upper === 'OR') { tokens.push({ t: 'or', v: 'OR' }); continue }
    if (upper === 'NOT') { tokens.push({ t: 'not', v: 'NOT' }); continue }
...
function andJoin(a: QueryNode, b: QueryNode): QueryNode {
  const nodes = [
    ...(a.type === 'and' ? a.nodes : [a]),
    ...(b.type === 'and' ? b.nodes : [b])
  ]
...
function parseTokens(tokens: RawToken[]): QueryNode | null {
  ...
  const startsPrimary = (tok?: RawToken) =>
    !!tok && (tok.t === 'term' || tok.t === 'phrase' || tok.t === 'qualifier' || tok.t === 'lparen' || tok.t === 'not')
  ...
  function parsePrimary(): QueryNode | null {
    ...
    if (tok.t === 'term') { pos++; return { type: 'term', value: tok.v } }
    if (tok.t === 'phrase') { pos++; return { type: 'term', value: tok.v, phrase: true } }
    if (tok.t === 'qualifier') { pos++; return { type: 'qualifier', key: tok.k!, value: tok.v } }
    pos++ // stray rparen / or
    return null
  }
...
function typeAlias(value: string): ResultType | undefined {
  switch (value.toLowerCase()) {
    case 'repo': case 'repos': case 'repository': case 'repositories': return 'repos'
    case 'code': return 'code'
    case 'issue': case 'issues': case 'pr': case 'prs': case 'pull': case 'pulls': case 'pullrequest': return 'issues'
    case 'user': case 'users': case 'org': case 'orgs': return 'users'
    case 'discussion': case 'discussions': return 'discussions'
...
function stripControls(node: QueryNode | null, meta: ParsedQuery): QueryNode | null {
  ...
  if (node.type === 'qualifier') {
    ...
    if (key === 'provider' || key === 'forge') {
      for (const p of val.split(',').map(s => s.trim().toLowerCase()).filter(Boolean)) {
    ...
    if (key === 'is') {
      const rt = typeAlias(val)
      if (rt) { if (!meta.resultTypes.includes(rt)) meta.resultTypes.push(rt); return null }
      return node // is:open / is:closed / is:merged etc. stay for the provider
    }
    if (key === 'sort') { meta.sortKey = val.toLowerCase(); return null }
    if (key === 'order') { meta.sortOrder = val.toLowerCase() === 'asc' ? 'asc' : 'desc'; return null }
    return node
  }
  if (node.type === 'not') {
...
export function collectQualifiers(
  ...
  else if (node.type === 'and' || node.type === 'or') node.nodes.forEach(c => collectQualifiers(c, out, negated))
  return out
}
