...
interface ProviderView {
  ...
  copy: string
  state: ConnState
  button: { label: string, icon: string, color: 'neutral' | 'primary', variant: 'outline' | 'solid' }
  connect: () => void
}
  ...
  <div class="space-y-6">
    ...
    <UCard v-for="p in providers" :key="p.id">
      ...
      <p v-if="p.state === 'needs-auth'" class="mt-3 text-xs text-muted">
        Your accounts stay linked to your identity, but the OAuth token that
        authorizes API access lives only on each device for security. Authorizing
        here is usually a single click — {{ p.label }} remembers you already approved koinon.
      </p>
    </UCard>
